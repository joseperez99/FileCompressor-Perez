...
def compress(file_path_source:str, file_path_destination:str, compressProgressBar:Progressbar, compressPercentTxt:StringVar, compressBtn:Button):
    
    try:
        print("> Compression started")
        start_time = time.time()
        compressBtn.config(state=DISABLED)
        #------------------ PROGRESS BAR ------------------#
        totalTasks = 10000
        completedTasks = 0
        ...
        percent = round((completedTasks / totalTasks) * 100, 1)
        compressProgressBar['value'] = percent
        compressPercentTxt.set(f'{int(percent)}%')
        #--------------------------------------------------#
